...
from .mcst_performance_response_parser import (
    McstPerformanceResponseParserProtocol,
    default_mcst_performance_response_parser,
)
from .settings import McstPerformanceSettings


class McstPerformanceGateway(Protocol):
    def request(self, operation: str, params: Mapping[str, Any]) -> Dict[str, Any]:
        """Fetch normalized data from the MCST performance API."""


@dataclass(frozen=True)
class UrllibMcstPerformanceGateway:
    ...
    def request(self, operation: str, params: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise McstPerformanceError(f"MCST performance API returned HTTP {exc.code}: {message}") from exc
        except error.URLError as exc:
            raise McstPerformanceError(f"MCST performance API request failed: {exc.reason}") from exc
...
def default_gateway_response_parser() -> McstPerformanceResponseParserProtocol:
    return default_mcst_performance_response_parser()
