...
from .arko_event_response_parser import ArkoEventResponseParserProtocol, default_arko_event_response_parser
from .exceptions import ArkoEventError
from .settings import ArkoEventSettings


class ArkoEventGateway(Protocol):
    def request(self, operation: str, params: Mapping[str, Any]) -> Dict[str, Any]:
        """Fetch normalized data from the ARKO event API."""


@dataclass(frozen=True)
class UrllibArkoEventGateway:
    ...
    def request(self, operation: str, params: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ArkoEventError(f"ARKO event API returned HTTP {exc.code}: {message}") from exc
        except error.URLError as exc:
            raise ArkoEventError(f"ARKO event API request failed: {exc.reason}") from exc
...
def default_gateway_response_parser() -> ArkoEventResponseParserProtocol:
    return default_arko_event_response_parser()
