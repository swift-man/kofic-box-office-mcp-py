...
from .constants import BOX_OFFICE_ENDPOINT, DATASET_NAME, DATASET_URL, KNOWN_RESULT_MESSAGES, SUCCESS_RESULT_CODES
...
class ResponseParser(Protocol):
    def parse(
        ...
        *,
        ...
        """Parse and normalize a KOFIC box-office API response."""


@dataclass(frozen=True)
class KoficBoxOfficeResponseParser:
    def parse(
        self,
        *,
        operation: str,
...
def parse_payload(*, raw_body: str, content_type: str) -> Dict[str, Any]:
    ...
    raise KoficBoxOfficeError("Expected JSON or XML from the KOFIC box-office API but received a different payload.")
...
def normalize_api_payload(
    *,
    ...
    if result_code and result_code not in SUCCESS_RESULT_CODES:
        ...
        raise KoficBoxOfficeError(f"KOFIC box-office API error {result_code}: {friendly_message}")
    ...
    return {
        ...
        "operation": operation,
        "operation_path": BOX_OFFICE_ENDPOINT if not operation else operation,
...
def _parse_json(raw_body: str) -> Dict[str, Any]:
    try:
        ...
        raise KoficBoxOfficeError("Failed to decode JSON returned by the KOFIC box-office API.") from exc

    if not isinstance(payload, dict):
        raise KoficBoxOfficeError("Expected a JSON object from the KOFIC box-office API.")
    return payload
...
def _parse_xml(raw_body: str) -> Dict[str, Any]:
    try:
        ...
        raise KoficBoxOfficeError("Failed to decode XML returned by the KOFIC box-office API.") from exc
