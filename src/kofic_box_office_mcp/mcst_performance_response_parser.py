...
from .mcst_performance_constants import (
    ...
    MCST_PERFORMANCE_OPERATION_PATH,
)


class McstPerformanceResponseParserProtocol(Protocol):
    def parse(
        self,
        *,
        operation: str,
        ...
        """Parse and normalize an MCST performance API response."""


@dataclass(frozen=True)
class McstPerformanceResponseParser:
    def parse(
        self,
        *,
        operation: str,
...
def default_mcst_performance_response_parser() -> McstPerformanceResponseParserProtocol:
    return McstPerformanceResponseParser()
...
def parse_payload(*, raw_body: str, content_type: str) -> Dict[str, Any]:
    ...
    raise McstPerformanceError("Expected JSON or XML from the MCST performance API but received a different payload.")
...
def normalize_api_payload(
    *,
    ...
    if result_code and result_code not in SUCCESS_RESULT_CODES:
        ...
        raise McstPerformanceError(f"MCST performance API error {result_code}: {friendly_message}")
    ...
    return {
        ...
        "operation": operation,
        "operation_path": MCST_PERFORMANCE_OPERATION_PATH if not operation else operation,
...
def _parse_json(raw_body: str) -> Dict[str, Any]:
    try:
        ...
        raise McstPerformanceError("Failed to decode JSON returned by the MCST performance API.") from exc

    if not isinstance(payload, dict):
        raise McstPerformanceError("Expected a JSON object from the MCST performance API.")
    return payload
...
def _parse_xml(raw_body: str) -> Dict[str, Any]:
    try:
        ...
        raise McstPerformanceError("Failed to decode XML returned by the MCST performance API.") from exc
